...
pub async fn process_message(config: Config, message: &str) -> Result<String> {
    ...
    if config.browser.enabled {
        ...
        tool_descs.push((
            "browser",
            "Automate web browsing (open, navigate, click, extract).",
        ));
    }
    if config.http_request.enabled {
        tool_descs.push((
            "http_request",
            "Make HTTP requests to external APIs (GET/POST/etc).",
        ));
    }
    if config.web_fetch.enabled {
        tool_descs.push(("web_fetch", "Fetch a web page and return markdown/text."));
    }
    if config.web_search.enabled {
        tool_descs.push((
            "web_search_tool",
